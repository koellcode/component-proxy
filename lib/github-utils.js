...
function RawGithubCache(cachePath, maxCacheAge) {
  this._cachePath = cachePath;
  this._maxCacheAge = maxCacheAge;
}

RawGithubCache.init = function(storageDir, maxCacheAge) {
  if(maxCacheAge === undefined || maxCacheAge === null) {
  ...
  return new RawGithubCache(cachePath, maxCacheAge);
}
...
RawGithubCache.prototype.get = function(user, repo, tree, repoPath, callback) {
  ...
  // If the .cache file is MAX_CACHE_AGE seconds or younger then use the file on the filesystem
  var currentTimestamp = new Date().getTime();
  var age = (currentTimestamp - cacheData.timestamp) / 1000;
  if(age > this._maxCacheAge) {
    // Otherwise, make a conditional request for the data
    return this._downloadAndCache(user, repo, tree, repoPath, cacheData, callback);
  }
  console.log("Using local cache %s", localPath);
...
RawGithubCache.prototype._downloadAndCache = function(user, repo, tree, repoPath, cacheData, callback) {
  ...
  console.log("Attempting to retrieve `%s` into temp path `%s`", githubURL, tempPath);
...
RawGithubCache.prototype._updateCache = function(user, repo, tree, repoPath, res, localPath) {
  ...
  if(res.statusCode === 200) {
    console.log("Updating latest version to %s", localPath);
    // Ensure the directory exists
    utils.ensureDir(path.dirname(localPath));
    // Copy the file to the localPath
    res.body.copyTo(localPath);
  } else {
    console.log("Version did not change for %s", localPath);
  }
  // Update the cache data
  var cacheData = {
 ...
 */
function PrivateGithubCache(oauthToken, cachePath, maxCacheAge) {
...
PrivateGithubCache.init = function(oauthToken, storageDir, maxCacheAge) {
  if(maxCacheAge === undefined || maxCacheAge === null) {
    maxCacheAge = MAX_CACHE_AGE;
  }
  var storagePath = utils.expandAndResolve(storageDir);
  ...
  // Ensure the cache path
  utils.ensureDir(cachePath);
  return new PrivateGithubCache(oauthToken, cachePath, maxCacheAge);
}
...
PrivateGithubCache.prototype.get = function(user, repo, tree, repoPath, callback) {
  ...
  // Load the repo's cache data
  var cacheData = this._loadRepoCacheData(user, repo, tree);
  var currentTimestamp = new Date().getTime();
  var age = (currentTimestamp - cacheData.timestamp) / 1000;
  if(age > this._maxCacheAge) {
    return this._downloadAndExtractRepoZip(user, repo, tree, cacheData, finish);
  }
  console.log("Using local cache for private repo %s/%s@%s", user, repo, tree);
  return finish();
}
...
PrivateGithubCache.prototype._downloadAndExtractRepoZip = function(user, 
  ...
  var downloadZip = function(err, treeData, zipURL) {
    ...
    // Create a temporary filepath
    var tempPath = temp.path();
    console.log("Retrieving zipball for repo `%s/%s@%s` into temp path `%s`", user, repo, tree, tempPath);
    // Create a stream for the temp file
    var tempStream = fs.createWriteStream(tempPath);
    var removeTempFile = function() {
      fs.unlinkSync(tempPath);
    ...
    request.get(zipURL)
      .end(function(err, res) {
        ...
        var finish = function(err, zipRef) {
          if(res.statusCode !== 200) {
            console.log("Could not access %s", zipURL);
            // Clean up temp
            removeTempFile();
            return callback(new Error('NotFound'));
          }
...
PrivateGithubCache.prototype._extractRepoZip = function(user, repo, tree, zipPath, cb) {
  ...
  fs.createReadStream(zipPath)
    ...
    .on('entry', function(entry) {
      ...
      var entrySplitPath = entry.path.split(path.sep);
      ...
      entry.pipe(fs.createWriteStream(entryAbsPath));
    })
    .on('close', function() {
      cb();
    });
